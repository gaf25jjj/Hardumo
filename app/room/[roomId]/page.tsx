...
export default function RoomPage() {
  ...
  const handleCopyRoomCode = async () => {
    ...
    setInviteStatus('Код комнаты скопирован');
  };
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => setInviteStatus(''), 1800);
  ...
  return <main className='min-h-screen p-4 space-y-3'>
    ...
    <aside className='panel p-3 min-h-[260px] max-h-[50vh] overflow-hidden flex flex-col'>
      <h2 className='text-sm font-medium mb-2'>Чат комнаты</h2>
      <div className='flex-1 overflow-y-auto space-y-2 pr-1'>
        {messages.map((m:any)=><div key={m.id} className='text-sm'><span className='text-white/75'>{m.user}:</span> {m.text}</div>)}
      </div>
      <form onSubmit={(e: FormEvent) => { e.preventDefault(); const text = messageText.trim(); if (!text) return; socketRef.current?.emit('chat:send', { text }); setMessageText(''); }} className='mt-2 flex gap-2'>
        <input value={messageText} onChange={(e)=>setMessageText(e.target.value)} placeholder='Написать сообщение…' className='flex-1 rounded bg-black/30 px-2 py-2 text-sm' />
        <button className='rounded bg-accent px-3 py-1'>Отправить</button>
      </form>
    </aside>
  </main>;
}
