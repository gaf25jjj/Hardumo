...
export class PlayerSyncController {
  ...
  queueOrApplyRemoteState = (state: PlaybackState) => {
    ...
    if (state.seq && state.seq <= this.o.lastSeqRef.current) return;
    if (state.seq) this.o.lastSeqRef.current = state.seq;
    ...
    if (!this.o.isHostRef.current && !this.o.userActivatedSyncRef.current) {
      this.o.pendingRemoteStateRef.current = state;
  ...
  applyRemoteState = async (state: PlaybackState) => {
    ...
    if (state.provider === 'vk') {
      console.log('[VK GUEST APPLY]', { targetTime: target, isPlaying: state.isPlaying, seq: state.seq });
    }
